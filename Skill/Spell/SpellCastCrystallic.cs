...
{
    [DeclareBoxGroup("Shards")]
    [DeclareBoxGroup("Staff Slam")]
    [DeclareBoxGroup("Imbue Hit")]
    public class SpellCastCrystallic : SpellCastCharge
    {
        ...
        [Group("Imbue Hit")]
        public float minImbueHitVelocity = 7.5f;
        ...
        [Group("Shards")]
        public float defaultShardLifetime = 0.75f;
        
        [Group("Shards")]
        public float throwVelocityMultiplier = 2.5f;

        [Group("Shards")]
        public Vector2 minMaxShardshotAngle = new(25f, 40f);
        ...
        public override void LoadSkillPassives(int skillCount)
        {
            ...
            AddModifier(this, Modifier.Efficiency, Mathf.Max(minMaxShardshotAngle.x, minMaxShardshotAngle.y - efficiencyPerSkill * skillCount));
        }
        ...
        public override void Throw(Vector3 velocity)
        {
            ...
            {
                FireShard(shardEffectData, position, direction * (velocity.magnitude * throwVelocityMultiplier), ShardLifetime, 1.0f, shard =>
                {
                    total++;
                    shards.Add(shard);
                    if (total == ShardCount)
        ...
        public override void UpdateImbue(float speedRatio)
        {
            ...
            if (imbue != null && Dye.rainbowMode)
                foreach (Effect effect in imbueEffect.effects)
                    if (effect is EffectShader effectShader)
                        effectShader.SetMainGradient(Utils.MakeShiftingGradient());
        }
        ...
        {
            if (Time.time - lastTime > cooldown && collisionInstance.impactVelocity.sqrMagnitude > minImbueHitVelocity * minImbueHitVelocity)
            {
                lastTime = Time.time;
