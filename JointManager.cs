...
public static class JointManager
{
    ...
    public static T CreateJoint<T>(object owner, string id, Func<T> jointFactory) where T : Joint
    {
        ...
        if (ownerJoints.TryGetValue(id, out var existingJoint)) UnityEngine.Object.Destroy(existingJoint);
        T newJoint = jointFactory();
    ...
    {
        joint = null;
        if (joints.TryGetValue(owner, out var ownerJoints) && ownerJoints.TryGetValue(id, out var foundJoint))
        {
            joint = foundJoint as T;
            return joint != null;
        }
        return false;
    }
    ...
    {
        if (joints.TryGetValue(owner, out var ownerJoints) && ownerJoints.TryGetValue(id, out var joint))
        {
    ...
    public static void DestroyAllJoints(object owner)
    {
        if (joints.TryGetValue(owner, out var ownerJoints))
        {
            foreach (var joint in ownerJoints.Values) UnityEngine.Object.Destroy(joint);
            joints.Remove(owner);
        }
    }
}